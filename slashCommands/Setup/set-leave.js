...
module.exports = {
	name: "set-leave", //the command name for the Slash Command
  category: "Setup",
	description: "Setup the  leave plugin", //the command description for Slash Command Overview
	cooldown: 1,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!	
		...
		{"Channel": { name: "what_channel", description: "Which channel should I send leave messages?", required: true }}, //to use in the code: options.getChannel("what_channel")
	...
	run: async (client, interaction) => {
      ...
      const message_input = new TextInputBuilder().setCustomId('message').setLabel('The message to send when someone leaves').setStyle(TextInputStyle.Paragraph)
       // .setValue('Welcome to {server}, {user} [{username}]! You are member number {membercount}.')
        .setValue('{user} left {server}, we now have {membercount} members')
          .setRequired(false)
          .setMinLength(1)
  ...
  actionRows.push(
    ...
    new ActionRowBuilder().addComponents(tagline_text_input)
      ...
      const modal = new ModalBuilder()
        ...
        .setTitle('Setup Leaver!')
        .addComponents(actionRows);

      await interaction.showModal(modal);
       const filter_modal = (interaction) => interaction.customId === 'leave_modal';
...
if (data) {
  if (!data.leave_data || typeof data.leave_data !== 'object') {
    data.leave_data = {};
  }
  data.leave_data.message = message_input_data;
  data.leave_enabled = isAccepted('true');
  data.leave_data.channel_id = to_send_channel.id;
  data.leave_data.image_enabled = isAccepted(image_enabled);
  data.leave_data.image_data = {
  ...
  data.leave_data.embed = isAccepted(embed_enabled);
  data.leave_data.embed_color = parseHexColor(embed_hex) || 'FFFFFF';
  data.leave_data.image_embed = isAccepted(embed_image);
  ...
  await client.db.set(`data_${guild.id}`, {
    ...
    "leave_data": {
      ...
      const confirm_embed = new EmbedBuilder()
           .setTitle(`Leave Setup Success`)
        .setColor(`#${embed_hex}`)
        .setFooter({ text: ee.footertext, iconURL: ee.footericon})
      .setDescription("This will be the message if someone left the server \n \n" + `> ${await replaceMessagePlaceholders(message_input_data, member)}`)
      .addFields({
        name: "Leave Channel",
